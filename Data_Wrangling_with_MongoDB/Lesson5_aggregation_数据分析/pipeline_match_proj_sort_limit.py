# -*- coding: utf-8 -*-
"""
Created on Sat Dec  5 17:14:43 2015
...
def make_pipeline():
    ...
    pipeline = [ ]
    match_pipe = {"$match" : { "user.time_zone" : "Brasilia", 
                               "user.statuses_count" : {"$gte" : 100}
                             }
                 }
    proj_pipe = {"$project" : { "followers" : "$user.followers_count",
                                "screen_name" : "$user.screen_name",
                                "tweets" : "$user.statuses_count"
                              }
                }
    sort_pipe = {"$sort" : {"followers" : -1
                            
                           }
                }
    limit_pipe = {"$limit" : 1
                 }
    pipeline += [match_pipe, proj_pipe, sort_pipe, limit_pipe]
    return pipeline
